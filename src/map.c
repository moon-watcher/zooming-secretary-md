...
void map_run( void )
{
	...
	VDP_setEnable( FALSE );
	    ...
	    VDP_drawImageEx( APLAN, &GamePause_APLAN, TILE_ATTR_FULL( PAL2, TRUE, FALSE, FALSE, tileindexC ), 0, VDP_getScreenHeight()>>TILE_BITS, FALSE, FALSE );
		...
		for ( y = 0; y < 28; y++ )
		{
			for ( x = 0; x < 40; x++ )
			{
				...
				VDP_setTileMapXY( APLAN, TILE_ATTR_FULL( PAL1, prioA, FALSE, FALSE, tileindexA + posA ), x, y );
				VDP_setTileMapXY( BPLAN, TILE_ATTR_FULL( PAL0, prioB, FALSE, FALSE, tileindexB + posB ), x, y );

				switch ( type )
				{
					case TILE_COFFEE:
						coffeeInitialize( (x << TILE_BITS) -1, (y << TILE_BITS) -1 );
						break;

					case TILE_PLAYER:
						player_init( x << TILE_BITS, y << TILE_BITS );
						break;

					case TILE_CEILING_FAN:
                        ceilingFanAdd( x << TILE_BITS, (y << TILE_BITS)-1 );
                        break;
					...
					case TILE_TOPIC_3:
					case TILE_TOPIC_4:
						topic_init( x << TILE_BITS, y << TILE_BITS, type - TILE_TOPIC_1 );
						break;

					case TILE_PHONE:
						phoneAdd( ( x << TILE_BITS ) - ( ( IS_LEVEL_BONUS ) ? 4 : 0), (y << TILE_BITS)+1 );
						break;
					...
					case NPC_GEEK:
					case NPC_DIBROV:
						npcInitialize( x << TILE_BITS, y << TILE_BITS, type );
						break;
					case NPC_GHOST:
						npcInitialize( 16, VDP_getScreenHeight(), type );
						break;
				}
